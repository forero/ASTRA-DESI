...
def main():
    ...
    for i in range(len(void_ids)):
        ...
        theta0 = np.deg2rad(void_ra[i] - args.ra_center_deg)
    ...
    ra_tick0 = np.ceil(ra_min / args.ra_tick_step) * args.ra_tick_step
    ra_tick1 = np.floor(ra_max / args.ra_tick_step) * args.ra_tick_step
    if ra_tick0 <= ra_tick1:
        ra_ticks = np.arange(ra_tick0, ra_tick1 + 0.5 * args.ra_tick_step, args.ra_tick_step)
        ax.set_xticks(np.deg2rad(ra_ticks - args.ra_center_deg))
        ax.set_xticklabels([rf'${t:.0f}^\circ$' for t in ra_ticks], fontsize=11)

    if args.rtick_step > 0:
        rticks = np.arange(args.rtick_step, args.rmax_mpc + 0.5 * args.rtick_step, args.rtick_step)
        ax.set_yticks(rticks)
        ax.set_yticklabels([rf'${int(t):d}$' for t in rticks], fontsize=11)
        ax.set_rlabel_position(theta_min_deg + 5.0)

    ax.tick_params(colors='white', labelsize=11)
    ax.set_title(rf'$\mathrm{{BGS\ NGC\ galaxies\ +\ void\ groups}},\ \delta={args.slice_width_deg:.0f}^\circ'
                 rf'\ (\mathrm{{DEC\ center}}={dec0:.2f}^\circ)$',
                 fontsize=17, pad=28, color='white',)
    rlabel_theta_deg = theta_min_deg - 2.5
    ax.text(np.deg2rad(rlabel_theta_deg), args.rmax_mpc * 1.13,
            r'$r\,[\mathrm{Mpc}]$', fontsize=16,
            color='white', rotation=-22,
            rotation_mode='anchor', ha='left', va='center',
            clip_on=False,)
    ax.text(0.5, 0.995, r'$\alpha\ (\mathrm{RA})$',
            transform=ax.transAxes, ha='center',
            va='top', fontsize=13, color='white',)
    ...
    if not args.hide_legend:
        ...
        if args.plot_galaxies:
            handles.append(
                Line2D([0], [0],
                        ...
                        label='Sheet/Filament/Knot', alpha=0.9,))
        if member_theta is not None and member_gid is not None and len(member_gid) > 0:
            handles.append(
        ...
        handles.append(
            Line2D([0], [0], marker='o', linestyle='', markersize=7,
                   markerfacecolor='none', markeredgecolor='white',
                   label='Void groups', alpha=0.9,))
        leg = fig.legend(handles=handles, labels=[h.get_label() for h in handles],
                         loc='upper right', bbox_to_anchor=(0.985, 0.965), framealpha=0.6, ncol=1, fontsize=10,)
    ...
    print('--- BGS NGC Wedge (Void Groups) Summary ---')
    ...
    if z_kind == 'redshift':
        if len(void_z) > 0:
            print(f'Void z range: {np.nanmin(void_z):.6f} .. {np.nanmax(void_z):.6f} | '
                  f'r range [Mpc]: {np.nanmin(void_r_mpc):.3f} .. {np.nanmax(void_r_mpc):.3f}')
    else:
        if len(void_r_mpc) > 0:
            print(f'Void r range [Mpc]: {np.nanmin(void_r_mpc):.3f} .. {np.nanmax(void_r_mpc):.3f}')
