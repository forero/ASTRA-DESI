...
def compute_cartesian(tbl):
    ...
    """
    # dist = Planck18.comoving_distance(tbl["Z"])
    # sc = SkyCoord(ra=tbl["RA"] * u.deg, dec=tbl["DEC"] * u.deg, distance=dist)
...
def process_real(real_tables, tracer, zone, north_rosettes):
    ...
    try:
        ...
        raise KeyError(f"No data for tracer '{tracer}' in hemisphere '{hemi}'")
...
def generate_randoms(random_tables, tracer, zone, north_rosettes, n_random, real_count):
    ...
    try:
        ...
        raise KeyError(f"No random data for tracer '{tracer}' in hemisphere '{hemi}'")
    ...
    for j in range(n_random):
        ...
        sel = zone_tables[idx]
        #* use np.random.default_rng instead of random.sample (recommended api)
